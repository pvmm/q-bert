...
void PSG_SetRegister(uint8_t reg, uint8_t value)
{
	...
	uint8_t* ptr = (uint8_t*)g_PSG_Regs;
	ptr += reg;
	*ptr = value;
...
void PSG_SetTone(uint8_t chan, u16 period)
{
#if (PSG_ACCESS == PSG_INDIRECT)
	...
	ptr += (chan * 2);
	*ptr = (uint8_t)period;
	*++ptr = (uint8_t)(period >> 8);

#else // if (PSG_ACCESS == PSG_DIRECT)

	uint8_t reg = (chan * 2);
	PSG_VAR_REG = reg;
	PSG_VAR_WRITE = (uint8_t)period;
	PSG_VAR_REG = ++reg;
	PSG_VAR_WRITE = (uint8_t)(period >> 8);
...
void PSG_SetNoise(uint8_t period)
{
	...
	uint8_t* ptr = (uint8_t*)g_PSG_Regs + PSG_REG_NOISE;
	*ptr = period;
...
void PSG_SetMixer(uint8_t mix)
{
	...
	uint8_t* ptr = (uint8_t*)g_PSG_Regs + PSG_REG_MIXER;
...
void PSG_SetVolume(uint8_t chan, uint8_t vol)
{
	...
	uint8_t* ptr = (uint8_t*)g_PSG_Regs + PSG_REG_AMP_A;
	ptr += chan;
	*ptr = vol;
...
{
#if (PSG_ACCESS == PSG_INDIRECT)
	uint8_t mix = g_PSG_Regs.Mixer;
#else // if (PSG_ACCESS == PSG_DIRECT)
	PSG_VAR_REG = PSG_REG_MIXER;
	uint8_t mix = PSG_VAR_READ;
#endif
...
{
#if (PSG_ACCESS == PSG_INDIRECT)
	uint8_t vol = g_PSG_Regs.Volume[chan];
#else // if (PSG_ACCESS == PSG_DIRECT)
	PSG_VAR_REG = PSG_REG_AMP_A + chan;
	uint8_t vol = PSG_VAR_READ;
#endif
